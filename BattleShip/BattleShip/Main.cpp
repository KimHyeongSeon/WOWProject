...
#define MULTI_PLAY 2
#define SERVER_ADDRESS "http://192.168.0.2:3000"   //"http://mighty-peak-9247.herokuapp.com/"
#define BLACK 0 
#define BLUE 1 
#define GREEN 2 
#define CYAN 3 
...
void position()
{
	...
	do
	{
		...
		dist2 = distance_check(column, column2, 3);//horizontal 수평
		found = 0;
		if (dist1 == 1) //if the ship is placed vertically 전함이 수직일떄
		{
			if (row > row2)
			{
				for (i = 0; i < 3; i++)
		...
		else if (dist2 == 1) //if the ship is placed horizontally 전함의 배치가 수평일떄
		{
			if (column > column2)
			{
				for (i = 0; i < 3; i++)
	...
	do
	{
		...
		found = 0;
		if (dist1 == 1) //if the ship is placed vertically 전함의 배열이 수직일떄
		{
			if (row > row2)
			{
				for (i = 0; i < 2; i++)
...
void cp_attack()
{
	...
	case 4://when hit
		...
		case 'A':
			...
			if (HIT == 5)	//if the ship is destroyed resets the counter 전함이 공격되었을떄 카운터 리셋
			{
				HIT = 0;
			}
			break;
		case 'B':
			...
			if (HIT == 4)	//if the ship is destroyed resets the counter전함이 공격되었을떄 카운터 리셋
			{
				HIT = 0;
			}
			break;
		case 'D':
			...
			if (HIT == 3) //if the ship is destroyed resets the counter전함이 공격되었을떄 카운터 리셋
			{
				HIT = 0;
			}
			break;
		...
		case 'P':
			...
			if (HIT == 2) //if the ship is destroyed resets the counter전함이 공격되었을떄 카운터 리셋
			{
				HIT = 0;
			}
			break;
